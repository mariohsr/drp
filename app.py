...
def analisar_print(image_bytes, key):
    ...
    # Usando gemini-2.0-flash para maior velocidade
...
if uploaded_file:
    try:
        ...
        if "429" in str(e):
            st.error("⚠️ Limite de uso da IA atingido. Aguarde 60 segundos antes de tentar novamente.")
        elif "INVALID_ARGUMENT" in str(e):
            st.error("🔑 API Key inválida. Verifique sua chave no AI Studio.")
        else:
